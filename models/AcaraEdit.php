...
class AcaraEdit extends Acara
{
    ...
    // CSS class/style
    public $CurrentPageName = "acaraedit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("AcaraID") ?? Key(0) ?? Route(2)) !== null) {
                $this->AcaraID->setQueryStringValue($keyValue);
                $this->AcaraID->setOldValue($this->AcaraID->QueryStringValue);
            } elseif (Post("AcaraID") !== null) {
                $this->AcaraID->setFormValue(Post("AcaraID"));
                $this->AcaraID->setOldValue($this->AcaraID->FormValue);
            } else {
            ...
            if (Post("action", "") !== "") {
                ...
                $loadByQuery = false;
                if (($keyValue = Get("AcaraID") ?? Route("AcaraID")) !== null) {
                    $this->AcaraID->setQueryStringValue($keyValue);
                    $loadByQuery = true;
                } else {
                    $this->AcaraID->CurrentValue = null;
                }
            }
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("acaralist"); // No matching record, return to list
                        return;
                    }
                break;
            case "update": // Update
                $returnUrl = $this->getReturnUrl();
                if (GetPageName($returnUrl) == "acaralist") {
                ...
                if ($this->editRow()) { // Update record based on key
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "acaralist") {
                            ...
                            $returnUrl = "acaralist"; // Return list page content
                        }
                    }
                    if (IsJsonResponse()) {
                        $this->terminate(true);
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("acaralist"), "", $this->TableVar, true);
        $pageId = "edit";
