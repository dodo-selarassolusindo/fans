...
class FansAdd extends Fans
{
    ...
    // CSS class/style
    public $CurrentPageName = "fansadd";
        ...
        switch ($this->CurrentAction) {
            ...
            case "insert": // Add new record
                ...
                if ($this->addRow($rsold)) { // Add successful
                    ...
                    } elseif (GetPageName($returnUrl) == "fansview") {
    ...
    protected function getAddRow()
    {
        ...
        // TahunKelahiran
        $this->TahunKelahiran->setDbValueDef($rsnew, $this->TahunKelahiran->CurrentValue, false);

        // Kota
        $this->Kota->setDbValueDef($rsnew, $this->Kota->CurrentValue, false);

        // Profesi
        $this->Profesi->setDbValueDef($rsnew, $this->Profesi->CurrentValue, false);

        // Hobi
        $this->Hobi->setDbValueDef($rsnew, $this->Hobi->CurrentValue, false);

        // AcaraID
        $this->AcaraID->setDbValueDef($rsnew, $this->AcaraID->CurrentValue, false);

        // RadioID
        $this->RadioID->setDbValueDef($rsnew, $this->RadioID->CurrentValue, false);
    ...
    protected function restoreAddFormFromRow($row)
    {
        ...
        if (isset($row['TahunKelahiran'])) { // TahunKelahiran
            $this->TahunKelahiran->setFormValue($row['TahunKelahiran']);
        }
        if (isset($row['Kota'])) { // Kota
            $this->Kota->setFormValue($row['Kota']);
        }
        if (isset($row['Profesi'])) { // Profesi
            $this->Profesi->setFormValue($row['Profesi']);
        }
        if (isset($row['Hobi'])) { // Hobi
            $this->Hobi->setFormValue($row['Hobi']);
        }
        if (isset($row['AcaraID'])) { // AcaraID
            $this->AcaraID->setFormValue($row['AcaraID']);
        }
        if (isset($row['RadioID'])) { // RadioID
            $this->RadioID->setFormValue($row['RadioID']);
        }
