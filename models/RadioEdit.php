...
class RadioEdit extends Radio
{
    ...
    // CSS class/style
    public $CurrentPageName = "radioedit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("RadioID") ?? Key(0) ?? Route(2)) !== null) {
                $this->RadioID->setQueryStringValue($keyValue);
                $this->RadioID->setOldValue($this->RadioID->QueryStringValue);
            } elseif (Post("RadioID") !== null) {
                $this->RadioID->setFormValue(Post("RadioID"));
                $this->RadioID->setOldValue($this->RadioID->FormValue);
            } else {
            ...
            if (Post("action", "") !== "") {
                ...
                $loadByQuery = false;
                if (($keyValue = Get("RadioID") ?? Route("RadioID")) !== null) {
                    $this->RadioID->setQueryStringValue($keyValue);
                    $loadByQuery = true;
                } else {
                    $this->RadioID->CurrentValue = null;
                }
            }
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("radiolist"); // No matching record, return to list
                        return;
                    }
                break;
            case "update": // Update
                $returnUrl = $this->getReturnUrl();
                if (GetPageName($returnUrl) == "radiolist") {
                ...
                if ($this->editRow()) { // Update record based on key
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "radiolist") {
                            ...
                            $returnUrl = "radiolist"; // Return list page content
                        }
                    }
                    if (IsJsonResponse()) {
                        $this->terminate(true);
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("radiolist"), "", $this->TableVar, true);
        $pageId = "edit";
